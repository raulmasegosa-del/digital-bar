...
export default function WaiterActions({ restaurantId }: { restaurantId: string }) {
  ...
  async function sendCall(type: "waiter" | "bill", waiterDescription = "") {
    ...
    try {
      setLoading(true);
      await createServiceCall({ table, type, restaurantId, description: type === "waiter" ? waiterDescription : undefined });
      setDescription("");
  ...
  return <>
    <div className="mb-8 grid grid-cols-2 gap-3">
      <button onClick={() => setShowWaiterDialog(true)} disabled={loading} className="rounded-xl bg-sky-600 py-3 font-semibold text-white hover:bg-sky-700 disabled:opacity-50">🙋 Llamar al camarero</button>
    ...
    {showWaiterDialog && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
