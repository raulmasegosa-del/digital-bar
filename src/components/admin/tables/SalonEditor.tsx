...
function TableDescription({name,number}:{name:string|null;number:number}){
 const description=name && name !== `Mesa ${number}` ? name : null;
 return description ? <span className="mt-1 max-w-[122px] truncate rounded-md border border-zinc-700/70 bg-zinc-950/60 px-2 py-0.5 text-[10px] font-medium text-zinc-300" title={description}>{description}</span> : null;
}
...
export default function SalonEditor({slug,restaurantId,tables}:Props){
 ...
 <div className="relative min-h-[200vh] touch-none overflow-hidden bg-[radial-gradient(circle_at_1px_1px,#3f3f46_1px,transparent_1px)] [background-size:24px_24px]" onPointerMove={move} onPointerUp={stopDragging} onPointerCancel={stopDragging}>
