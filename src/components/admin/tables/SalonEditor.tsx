...
export default function SalonEditor({slug,restaurantId,tables}:Props){
 ...
 function rotateBar(e:React.PointerEvent){if(!editing)return;e.stopPropagation();setBarRotation(r=>r===0?90:0);}
 ...
 <div className="relative min-h-[620px] touch-none overflow-hidden bg-[radial-gradient(circle_at_1px_1px,#3f3f46_1px,transparent_1px)] [background-size:24px_24px]" onPointerMove={move} onPointerUp={()=>{setDragging(null);setDraggingBar(false)}} onPointerCancel={()=>{setDragging(null);setDraggingBar(false)}}>
   <div className="absolute -translate-x-1/2 -translate-y-1/2" style={{left:`${barPosition.x}%`,top:`${barPosition.y}%`}} onPointerDown={e=>{if(editing){e.preventDefault();setDraggingBar(true)}}}>
     <button type="button" aria-label="Girar barra 90 grados" onPointerDown={rotateBar} className={`relative flex h-20 w-[50vw] min-w-[280px] max-w-[720px] items-center justify-center rounded-2xl border-2 border-amber-700/60 bg-gradient-to-r from-amber-950 via-amber-900 to-amber-950 shadow-2xl shadow-black/30 transition-transform duration-300 ${editing?"cursor-grab border-amber-400/70":"pointer-events-none"}`} style={{transform:`rotate(${barRotation}deg)`}}>
