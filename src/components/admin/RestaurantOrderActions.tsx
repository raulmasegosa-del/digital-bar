...
type Props = { slug: string; orderId: string; status: OrderStatus; orderIds?: string[] };
...
export default function RestaurantOrderActions({ slug, orderId, status, orderIds = [] }: Props) {
  const [isPending, startTransition] = useTransition();

  function changeStatus(nextStatus: OrderStatus) {
    startTransition(async () => {
      try {
        await updateRestaurantOrderStatus(slug, orderId, nextStatus, orderIds);
