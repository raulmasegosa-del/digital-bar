...
type NavigationItem = {
  href: string;
  label: string;
  icon: "dashboard" | "products" | "categories" | "options" | "orders" | "tables" | "qr" | "settings";
};

type Props = { children: ReactNode; restaurant: { name: string; slug: string }; navigation: NavigationItem[] };

const icons = { dashboard: BarChart3, products: ShoppingBag, categories: Folder, options: SlidersHorizontal, orders: ClipboardList, tables: Table2, qr: QrCode, settings: Settings };
        ...
        <aside className="flex w-64 shrink-0 flex-col border-r border-zinc-800 bg-[#141311]">
          <div className="border-b border-zinc-800 px-6 py-7">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-amber-500/30 bg-amber-500/10 text-amber-400"><Utensils size={20} strokeWidth={1.8} /></div>
              <div><div className="text-sm font-semibold uppercase tracking-[0.22em] text-amber-500">Digital Bar</div><div className="mt-1 text-sm font-medium text-white">{restaurant.name}</div></div>
            </div>
