...
type PaymentWarning = { paymentMethod: PaymentMethod; statuses: string[]; orderCount: number; total: number };
...
export default function ServiceCallsBoard({ restaurantId }: { restaurantId: string }) {
  ...
  async function resolveBill(paymentMethod: PaymentMethod, confirmGrouped = false) {
    if (!paymentGroup) return;
    setIsResolving(true);
    try {
      const result = await resolveBillServiceCall(restaurantId, paymentGroup.calls.map((call) => call.id), paymentMethod, confirmGrouped);
      if (result.requiresConfirmation) {
        setPaymentWarning({ paymentMethod, statuses: result.statuses, orderCount: result.orderCount, total: result.total });
        return;
      }
      setPaymentWarning(null); setPaymentGroup(null); void loadCalls(); window.location.reload();
    } catch (error) { window.alert(error instanceof Error ? error.message : "No se pudo cerrar la mesa"); }
    finally { setIsResolving(false); }
  }

  if (groups.length === 0 && !paymentGroup && !paymentWarning) return null;
  ...
  return <>
    ...
    {paymentGroup && !paymentWarning && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="service-payment-title">
      <div className="w-full max-w-md rounded-2xl border border-zinc-700 bg-[#1a1917] p-5 shadow-2xl sm:p-6"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-500">Cerrar mesa {paymentGroup.table_number}</p><h2 id="service-payment-title" className="mt-1 text-2xl font-black text-white">¿Cómo han pagado?</h2><p className="mt-2 text-sm text-zinc-400">Selecciona el método de pago para cerrar la mesa y desactivar su sesión.</p><div className="mt-6 grid grid-cols-2 gap-3"><button type="button" disabled={isResolving} onClick={() => void resolveBill("cash")} className="min-h-28 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-4 text-center transition hover:border-emerald-400/60 hover:bg-emerald-500/20 disabled:opacity-50"><span className="block text-4xl">💵</span><span className="mt-2 block text-base font-black text-white">Efectivo</span></button><button type="button" disabled={isResolving} onClick={() => void resolveBill("card")} className="min-h-28 rounded-2xl border border-blue-500/30 bg-blue-500/10 px-4 py-4 text-center transition hover:border-blue-400/60 hover:bg-blue-500/20 disabled:opacity-50"><span className="block text-4xl">💳</span><span className="mt-2 block text-base font-black text-white">Tarjeta</span></button></div><button type="button" disabled={isResolving} onClick={() => setPaymentGroup(null)} className="mt-4 w-full rounded-xl border border-zinc-700 bg-zinc-800/60 px-4 py-3 text-sm font-semibold text-zinc-300 hover:bg-zinc-800">Cancelar</button></div>
    </div>}

    {paymentWarning && paymentGroup && <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true"><div className="w-full max-w-md rounded-2xl border border-amber-500/50 bg-[#1a1917] p-5 shadow-2xl sm:p-6"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-400">⚠️ Atención · Mesa {paymentGroup.table_number}</p><h2 className="mt-1 text-2xl font-black text-white">Hay pedidos en varios estados</h2><p className="mt-3 text-sm leading-6 text-zinc-400">Esta mesa tiene <strong className="text-white">{paymentWarning.orderCount} pedidos</strong> pendientes en estados diferentes:</p><div className="mt-4 flex flex-wrap gap-2">{paymentWarning.statuses.map((status) => <span key={status} className="rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-1.5 text-sm font-bold text-amber-200">{status}</span>)}</div><div className="mt-4 rounded-xl border border-zinc-700 bg-[#11100f] p-4"><p className="text-xs text-zinc-500">Total a cobrar</p><p className="mt-1 text-3xl font-black text-white">{paymentWarning.total.toFixed(2)} €</p><p className="mt-1 text-xs text-zinc-500">Pago seleccionado: {paymentWarning.paymentMethod === "card" ? "tarjeta 💳" : "efectivo 💵"}</p></div><p className="mt-4 text-sm font-semibold text-amber-200">Se cerrarán todos los pedidos de esta sesión al confirmar el cobro.</p><div className="mt-5 flex gap-3"><button type="button" disabled={isResolving} onClick={() => { setPaymentWarning(null); setPaymentGroup(null); }} className="flex-1 rounded-xl border border-zinc-700 px-4 py-3 font-semibold text-zinc-300">Cancelar</button><button type="button" disabled={isResolving} onClick={() => void resolveBill(paymentWarning.paymentMethod, true)} className="flex-1 rounded-xl bg-amber-600 px-4 py-3 font-bold text-white hover:bg-amber-500 disabled:opacity-50">Sí, cobrar todo</button></div></div></div>}
  </>;
}
