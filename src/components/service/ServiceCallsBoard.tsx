...
export default function ServiceCallsBoard({ restaurantId }: { restaurantId: string }) {
  ...
  return <section className="mb-5 rounded-2xl border-2 border-red-500/40 bg-red-950/30 p-4 shadow-[0_10px_35px_rgba(239,68,68,0.12)] sm:p-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{groups.map((group) => { const minutes = Math.max(0, Math.floor((now - new Date(group.latest.created_at).getTime()) / 60000)); const urgent = minutes >= 3; const count = group.calls.length; return <div key={group.key} className={`flex items-center justify-between gap-3 rounded-xl border-2 p-4 shadow-lg transition-all ${urgent ? "animate-pulse border-red-500 bg-red-100 text-red-950" : "border-red-300/50 bg-white text-zinc-900"}`}><div className="min-w-0"><div className="flex items-center gap-2"><p className="text-xl font-black">Mesa {group.table_number}</p><span title={`${count} avisos pendientes`} className="flex h-7 min-w-7 items-center justify-center rounded-full bg-red-600 px-2 text-xs font-black text-white">{count}</span></div><p className="mt-1 text-sm font-semibold">{group.type === "waiter" ? "🙋 Llamar al camarero" : "💶 Pedir la cuenta"}</p><p className={`mt-1 text-xs font-semibold ${urgent ? "text-red-700" : "text-zinc-500"}`}>⏱ {minutes} min · {count === 1 ? "1 aviso" : `${count} avisos`}</p></div><button onClick={async () => { await Promise.all(group.calls.map((call) => completeServiceCall(call.id))); void loadCalls(); }} className="shrink-0 rounded-xl bg-green-600 px-4 py-3 font-bold text-white shadow hover:bg-green-700 active:scale-95">Resuelto</button></div>; })}</div>
  </section>;
}
