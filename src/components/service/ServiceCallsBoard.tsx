...
export default function ServiceCallsBoard({ restaurantId }: { restaurantId: string }) {
  ...
  async function resolveGroup(group: CallGroup) {
    ...
    try {
      ...
      window.alert(error instanceof Error ? error.message : "No se pudo resolver el aviso");
    }
  }
  ...
  async function resolveBill(paymentMethod: PaymentMethod) {
    ...
    setIsResolving(true);
    try {
      await resolveBillServiceCall(restaurantId, paymentGroup.calls.map((call) => call.id), paymentMethod);
      setPaymentGroup(null);
      void loadCalls();
      window.location.reload();
    } catch (error) {
      window.alert(error instanceof Error ? error.message : "No se pudo cerrar la mesa");
    } finally {
  ...
  return <>
    ...
    {paymentGroup && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="service-payment-title">
      <div className="w-full max-w-md rounded-2xl border border-zinc-700 bg-[#1a1917] p-5 shadow-2xl sm:p-6">
        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-500">Cerrar mesa {paymentGroup.table_number}</p>
        <h2 id="service-payment-title" className="mt-1 text-2xl font-black text-white">¿Cómo han pagado?</h2>
        <p className="mt-2 text-sm text-zinc-400">Selecciona el método de pago para cerrar la mesa y desactivar su sesión.</p>
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button type="button" disabled={isResolving} onClick={() => resolveBill("cash")} className="min-h-28 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-4 text-center transition hover:border-emerald-400/60 hover:bg-emerald-500/20 disabled:opacity-50"><span className="block text-4xl">💵</span><span className="mt-2 block text-base font-black text-white">Efectivo</span></button>
        ...
        <button type="button" disabled={isResolving} onClick={() => setPaymentGroup(null)} className="mt-4 w-full rounded-xl border border-zinc-700 bg-zinc-800/60 px-4 py-3 text-sm font-semibold text-zinc-300 hover:bg-zinc-800">Cancelar</button>
      </div>
    </div>}
  </>;
}
