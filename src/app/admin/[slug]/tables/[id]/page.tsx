...
import TableDescriptionForm from "@/components/admin/tables/TableDescriptionForm";
...
export default async function TableDetailPage({ params }: Props) {
  ...
  const initialDescription = table.name && table.name !== `Mesa ${table.number}` ? table.name : "";

  return (
    <main className="space-y-8">
      <PageHeader title={`Mesa ${table.number}`} description={`${restaurant.name} · ${table.zone || "Sin zona"}`} />
      ...
      <TableDescriptionForm restaurantId={restaurant.id} tableId={table.id} slug={slug} initialDescription={initialDescription} />
      <section className="rounded-2xl border border-zinc-800 bg-[#181716] p-6">
        <div className="flex flex-col gap-2 border-b border-zinc-800 pb-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-medium text-zinc-400">Mesa {table.number}</p><h2 className="mt-1 text-xl font-semibold text-white">Lo servido / pendiente</h2></div><div className="text-left sm:text-right"><p className="text-xs uppercase tracking-wide text-zinc-500">Total abierto</p><p className="mt-1 text-2xl font-semibold text-amber-500">{total.toFixed(2)} €</p></div></div>
