...
const statusLabels: Record<OrderStatus, string> = {
  pending: "Recibido", preparing: "Preparando", ready: "Listo", served: "Servido", bill: "Cuenta", completed: "Finalizado", cancelled: "Cancelado",
};
const statusClasses: Record<OrderStatus, string> = {
  pending: "border-yellow-500/30 bg-yellow-500/15 text-yellow-300", preparing: "border-blue-500/30 bg-blue-500/15 text-blue-300", ready: "border-emerald-500/30 bg-emerald-500/15 text-emerald-300", served: "border-emerald-500/30 bg-emerald-500/15 text-emerald-300", bill: "border-amber-500/30 bg-amber-500/15 text-amber-300", completed: "border-zinc-600 bg-zinc-700/60 text-zinc-200", cancelled: "border-red-500/30 bg-red-500/15 text-red-300",
};
...
export default async function OrdersPage({ params }: Props) {
  ...
  if (!superAdmin) {
    const { data: membership, error: membershipError } = await supabase.from("restaurant_users").select("restaurant_id, role").eq("user_id", user.id).eq("restaurant_id", restaurant.id).in("role", ["owner", "staff"]).maybeSingle();
  ...
  return (
    <main className="min-h-screen bg-[#11100f] text-white">
      <div className="mx-auto max-w-[1800px] px-3 py-4 sm:px-5 sm:py-6 lg:px-7">
        <header className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div><div className="mb-2 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl border border-amber-500/25 bg-amber-500/10 text-amber-400"><ClipboardList size={20} strokeWidth={1.8} /></div><span className="text-[10px] font-semibold uppercase tracking-[0.28em] text-amber-500">Cocina · Operativa</span></div><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Pedidos</h1><p className="mt-1 text-sm text-zinc-400">{restaurant.name} · panel de pedidos en tiempo real</p></div>
          <div className="flex gap-2"><div className="rounded-xl border border-zinc-800 bg-[#181716] px-4 py-2.5"><p className="text-[9px] uppercase tracking-[0.18em] text-zinc-500">Activos</p><p className="mt-0.5 text-xl font-bold text-white">{activeCount}</p></div><div className="rounded-xl border border-zinc-800 bg-[#181716] px-4 py-2.5"><p className="text-[9px] uppercase tracking-[0.18em] text-zinc-500">Histórico</p><p className="mt-0.5 text-xl font-bold text-white">{historical.length}</p></div></div>
        </header>

        <ServiceCallsBoard />
        <div className="mb-5 h-px bg-gradient-to-r from-amber-500/50 via-zinc-700 to-transparent" />

        {orders.length === 0 ? <div className="rounded-2xl border border-zinc-800 bg-[#181716] px-6 py-20 text-center"><ClipboardList className="mx-auto h-12 w-12 text-zinc-700" /><h2 className="mt-5 text-xl font-semibold text-white">No hay pedidos todavía</h2><p className="mt-2 text-sm text-zinc-500">Cuando un cliente haga un pedido aparecerá aquí.</p></div> : <><div className="grid gap-4 xl:grid-cols-3 xl:items-start">{sections.map((section) => <section key={section.key} className={`min-w-0 rounded-2xl border p-3 sm:p-4 ${section.accent}`}><div className="sticky top-2 z-10 mb-4 rounded-xl border border-zinc-700/80 bg-[#1a1917]/95 px-4 py-3.5 shadow-lg backdrop-blur"><div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className={`h-3 w-3 shrink-0 rounded-full ${section.dot}`} /><div className="min-w-0"><h2 className="text-lg font-bold text-white">{section.title}</h2><p className="truncate text-[11px] text-zinc-500">{section.description}</p></div></div><span className="flex h-9 min-w-9 shrink-0 items-center justify-center rounded-full border border-zinc-600 bg-[#11100f] px-2 text-sm font-bold text-white">{section.orders.length}</span></div></div>{section.orders.length === 0 ? <div className="flex min-h-[180px] items-center justify-center rounded-xl border border-dashed border-zinc-700/70 bg-[#171614]/80 px-5 py-8 text-center text-sm text-zinc-600">No hay pedidos</div> : <div className="space-y-3">{section.orders.map((order, index) => <OrderCard key={order.id} order={order} slug={slug} restaurantName={restaurant.name} alternate={index % 2 === 0} />)}</div>}</section>)}</div>{historical.length > 0 && <section className="mt-6 rounded-2xl border border-zinc-800 bg-[#151413] p-4 sm:p-5"><div className="mb-4 flex items-end justify-between gap-4 border-b border-zinc-800/80 pb-4"><div><h2 className="text-lg font-semibold text-zinc-300">Histórico</h2><p className="mt-1 text-xs text-zinc-600">Pedidos finalizados o cancelados</p></div><span className="rounded-full border border-zinc-800 bg-[#181716] px-3 py-1 text-xs text-zinc-500">{historical.length}</span></div><div className="grid gap-3 lg:grid-cols-2">{sortByCreatedAt(historical).reverse().map((order, index) => <OrderCard key={order.id} order={order} slug={slug} restaurantName={restaurant.name} compact alternate={index % 2 === 0} />)}</div></section>}</>}
      </div>
    </main>
  );
}
